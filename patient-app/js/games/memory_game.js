...
class MemoryGame {
  constructor(containerId, onComplete, rlAgent = null) {
  ...
  start(level = 2, aiConfig = null) {
    console.log("[MEMORY START] start() called");
    console.log("[MEMORY START] level:", level);
    console.log("[MEMORY START] aiConfig:", aiConfig);
    this.level = level;
    let specs;
    if (aiConfig) {
      specs = { cardCount: aiConfig.pairs * 2, timeLimitSec: aiConfig.time_limit_sec, hintCount: aiConfig.hints, gridCols: aiConfig.pairs <= 3 ? 3 : (aiConfig.pairs <= 4 ? 4 : 5) };
      console.log("[MEMORY START] Using AI config (NOT onDeviceDDA)");
    } else {
      specs = onDeviceDDA.getLevelSpecs("memory", level);
      console.log("[MEMORY START] Using onDeviceDDA fallback (no AI config)");
    }
    ...
    console.log("[MEMORY START] New board generated");
    console.log("[MEMORY START] Card count:", this.totalPairs * 2);
    console.log("[MEMORY START] Total pairs:", this.totalPairs);
    console.log("[MEMORY START] Time limit:", this.timeLimitSec);
    console.log("[MEMORY START] Hints:", this.hintLimit);
    console.log("[MEMORY START] sessionEnded:", this.sessionEnded);
    console.log("[MEMORY START] totalAttempts:", this.totalAttempts);
    console.log("[MEMORY START] isLocked:", this.isLocked);

    voiceService.speak("memory_intro");
    this.render();
    this.startTimer();

    console.log("[MEMORY START] Board motifs:", this.cards.map(c => c.motifId));
    console.log("[MEMORY START] Timer started, timeRemaining:", this.timeRemaining);
  }
  ...
  // AI adaptation removed (now handled by local RL agent)
  ...
  async handleGameComplete() {
    ...
    const aiResult = this.rlAgent ? this.rlAgent.learnFromSession(metrics) : null;
    voiceService.speak("game_complete");
    this.showCompletionModal(metrics, aiResult);
  ...
  async handleTimeout() {
    ...
    await localDB.saveGameSession(metrics);
    const aiResult = this.rlAgent ? this.rlAgent.learnFromSession(metrics) : null;
    this.showCompletionModal(metrics, aiResult, true);
    syncManager.attemptSync();
  }

  showCompletionModal(metrics, aiResult, isTimeout = false) {
    const nextConfig = aiResult ? aiResult.final_configuration : { level: this.level, pairs: this.totalPairs, time_limit_sec: this.timeLimitSec, hints: this.hintLimit };
    const gotHarder = nextConfig.level > this.level;
    const gotEasier = nextConfig.level < this.level;
    const messageText = gotHarder ? "Great work! The next round will be a little more challenging."
      : gotEasier ? "No worries. We'll make the next round a little more comfortable."
        : "Good effort! Let's keep practicing.";
    const btnText = `Play Next Round (Level ${nextConfig.level})`;

    const modal = document.createElement('div');
    modal.id = 'memory-completion-modal';

    modal.innerHTML = `
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-surface/95 backdrop-blur-md px-margin-page overflow-y-auto">
        <div class="w-full max-w-xl bg-surface-container rounded-3xl border-4 border-primary p-8 flex flex-col items-center text-center gap-6 shadow-2xl my-8">
          ...
          <div class="w-full bg-secondary-container rounded-xl p-4 text-left border-2 border-secondary">
             <h3 class="font-bold text-on-secondary-container mb-2">AI Adaptation</h3>
             <ul class="text-on-secondary-container space-y-1 text-sm">
                ${aiResult ? `
                  <li><span class="font-bold">Mode:</span> Offline Reinforcement Learning</li>
                  <li><span class="font-bold">Model:</span> Q-Learning</li>
                  <li><span class="font-bold">Status:</span> ${aiResult.model_status}</li>
                  <li><span class="font-bold">Episode:</span> ${aiResult.training_observations}</li>
                  <li><span class="font-bold">Selected Action:</span> ${aiResult.selected_candidate}</li>
                  <li><span class="font-bold">Next Level:</span> ${aiResult.final_configuration.level}</li>
                ` : `
                 <li><span class="font-bold">AI Unavailable:</span> No RL Agent configured</li>
               `}
             </ul>
          </div>
    ...
    modal.querySelector('#btn-next-level').addEventListener('click', () => {
      console.log("[MEMORY NEXT] BUTTON CLICKED");
      console.log("[MEMORY NEXT] nextConfig:", nextConfig);
      console.log("[MEMORY NEXT] current level before start:", this.level);
      console.log("[MEMORY NEXT] CALLING start()");
      modal.remove();
      this.start(nextConfig.level, nextConfig);
    });
